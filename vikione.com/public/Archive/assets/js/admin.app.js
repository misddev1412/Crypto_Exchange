...
!function(t) {
    ...
    t(document).on("click", "a#clear-cache", function(e) {
        e.preventDefault(),
        t.get(clear_cache_url).done(t=>{
            cl(t),
            "success" == t.msg && (show_toast(t.msg, t.message, "ti ti-trash"),
    ...
    t(document).on("click", "a.et-item", function(a) {
        ...
        t.post(get_et_url, {
            ...
            e.html(t),
            init_inside_modal(),
            e.children(".modal").modal("show")
        }
        )
    }),
    $resendverifyemail = t(".resend-verify-email"),
    $resendverifyemail.on("click", function(e) {
        e.preventDefault();
    ...
    t(document).on("click", "a.user-action", function(a) {
        ...
        var n = t(this).data("type")
          ...
          , i = t(this).data("uid");
        "transactions" == n || "activities" == n || "referrals" == n ? (e.empty(),
        e.parent().find(".modal-backdrop").remove(),
        t.post(show_user_info, {
            uid: i,
            req_type: n,
            _token: csrf_token
        }).done(a=>{
            a.status && "die" == a.status ? show_toast(a.msg, a.message, "ti ti-lock") : (bs_modal_hide(t(this)),
            e.html(a),
            e.children(".modal").modal("show"))
        }
            ...
            a && t.post(view_user_url, {
                ...
                null != a.msg && show_toast(a.msg, a.message),
                cl(a),
                ("active_user" == n || (n = "suspend_user")) && t(this).fadeOut(200, function() {
                    t(this).remove()
                }),
                t(document).find(".status_user").find(".badge").removeAttr("class").addClass("badge badge-" + a.css + " ucap"),
                t(".more-menu-" + i).append('<li><a href="#" data-uid="' + i + '" data-type="' + a.status + '" class="user-action"><em class="fas fa-ban"></em>' + ("suspend_user" == n ? "Active" : "Suspend") + "</a></li>"),
                e.html(a),
                e.children(".modal").modal("show")
            }
            )
        ...
        t.post(get_kyc_url, {
            ...
            "kyc_settings" == o) {
                ...
                t(".modal-close").on("click", function(e) {
                    e.preventDefault(),
                    !0 === r ? confirm("You made some changes, \nDo you realy close without save?") && (bs_modal_hide(t(this)),
                    r = !1) : bs_modal_hide(t(this))
                })
            }
            init_inside_modal(),
            e.children(".modal").modal("show")
    ...
    t(document).on("click", ".kyc_action", function() {
        kid = t(this).data("id"),
        t("input#kyc_id").val(kid)
    }),
    t("#actionkyc .status-btn").on("click", function(e) {
        e.preventDefault();
        var a = t(this).data("val");
        t("#actionkyc .status-btn").removeAttr("style"),
        t(this).css("border", "2px solid #34425d"),
        t("#actionkyc input#status").val(a)
    });
    $quick_update = ".update_kyc",
    $kyc_form = "#kyc_status_form",
    t(document).on("click", $quick_update, function(e) {
        e.preventDefault();
        ...
        t.post(update_kyc_url, {
            ...
            "success" != t.msg && "warning" != t.msg || window.location.reload()
        }
        )
    }),
    t(document).on("click", "a.kyc_reject", function(e) {
        ...
        swal({
            ...
            e && t.post(update_kyc_url, {
                ...
                cl(e),
                show_toast("warning", e.message, "ti ti-trash"),
                i.find("span.badge").removeClass("badge-" + n).addClass("badge-danger"),
                i.find("span.badge").text("Rejected"),
                a.fadeOut(300),
                t(".more-menu-" + o).find(".kyc_approve").length < 1 && t(".more-menu-" + o).append('<li><a class="kyc_action" href="#" data-id="' + o + '" data-toggle="modal" data-target="#actionkyc"><em class="far fa-check-square"></em>Approve</a></li>'),
                t(".more-menu-" + o).find(".kyc_delete").length < 1 && t(".more-menu-" + o).append('<li><a href="javascript:void(0)" data-id="' + o + '" class="kyc_delete"><em class="fas fa-trash-alt"></em>Delete</a></li>')
            }
            )
        }
        )
    ...
    j.length > 0 && D.on("click", function() {
        confirm("Are you sure?") && (ajax_form_submit(j, !1),
        D.parents("form#update_tokens").submit())
    });
    ...
    if (S.length > 0) {
        ...
        ajax_form_submit(S, !1),
        S.find(".active_method").change(function() {
            var e = t(".active_method").val();
            e = e.toLowerCase(),
            S.find(".all_methods").removeAttr("disabled");
            var a = S.find("#pw-" + e);
            !a.is(":checked") && a.click(),
            a.attr("disabled", !0)
        })
    }
    ...
    var P = t("form#point_setting_form");
    if (P.length > 0) {
        var O = P.find(_button_submit);
        P.find(a).on("keyup change", function() {
            btn_actived(O)
        }),
        ajax_form_submit(P, !1)
    }

    var AF = t("form#affiliate_setting_form");
    if (AF.length > 0) {
        var OF = AF.find(_button_submit);
        AF.find(a).on("keyup change", function() {
            btn_actived(OF)
        }),
        ajax_form_submit(AF, !1)
    }
    ...
    t(document).on("click", ".stages-ajax-action", function(a) {
        ...
        var n = t(this)
          ...
          , s = o.action
          , r = o.stage;
        n.parents(".stage-action").find(".toggle-tigger").add(".toggle-class").removeClass("active"),
        "overview" == s && "undefined" != typeof stage_action_url && (i = stage_action_url),
        null !== i && r ? (o._token = csrf_token,
        t.post(i, o).done(function(t) {
            if (void 0 !== t.modal && t.modal)
                e.html(t.modal),
                init_inside_modal(),
                e.children(".modal").length > 0 && e.children(".modal").modal("show");
            else if (t.message) {
    ...
    t("a#update_stage").on("click", function(e) {
        e.preventDefault();
        var a, n = t(this).data("type"), o = t(this).data("id"), i = "";
        a = "active_stage" == n ? stage_active_url : stage_pause_url,
        "active_stage" == n ? i = "Once you make this stage active, other stage will inactive and stop sale on that stage." : "pause_stage" == n ? i = "Do you want to pause temporary your running sales and purchase option disabled?" : "resume_stage" == n && (i = "Do you want to resume your sales and contributor able to purchase token?"),
        swal({
            title: "Are you sure?",
            text: i,
            icon: "info",
            ...
            e && t.post(a, {
                ...
                cl(t),
                show_toast(t.msg, t.message, "ti ti-eye"),
                "success" == t.msg && window.location.reload()
            }
            ).fail(t=>{
                cl(t)
            }
    ...
    t(document).on("click", ".tnx-action", function() {
        var e = t(this)
          ...
          , n = e.data("type")
          , o = e.data("id");
        token = e.data("token") ? e.data("token") : 0,
        chk_adjust = e.data("_chk") ? e.data("_chk") : 0,
        adjusted_token = e.data("_adjusted") ? e.data("_adjusted") : 0,
        base_bonus = e.data("_b_bonus") ? e.data("_b_bonus") : 0,
        token_bonus = e.data("_t_bonus") ? e.data("_t_bonus") : 0,
        amount = e.data("_amount") ? e.data("_amount") : 0,
        swal_icon = "approved" == n ? "info" : "warning",
        swal_cta = "approved" == n ? "Approve" : "Yes",
        swal_ctac = "approved" == n ? "" : "danger",
        "approved" == n && null != amount && amount <= 0 ? show_toast("warning", "Invalid Received Amount!") : swal({
            title: "Are you sure?",
            ...
            null == i && "" != i || t.post(trnx_action_url, {
                ...
                i.status || ("approved" == n && (t("#tnx-item-" + o).find(".token-amount").text("+" + i.data.total_tokens),
                t("#tnx-item-" + o).find(".amount-pay").text("+" + i.data.amount),
                t("#ds-" + o).removeAttr("class").addClass("data-state data-state-approved"),
                t("#more-menu-" + o).html('<li><a href="' + base_url + "/admin/transactions/view/" + o + '"><em class="ti ti-eye"></em> View Details</a></li>')),
                "canceled" == n && (t("#more-menu-" + o).find("#canceled").fadeOut(400, function() {
                    t(this).remove()
                }),
                t("#ds-" + o).removeAttr("class").addClass("data-state data-state-canceled"),
                t("#more-menu-" + o).append('<li><a href="javascript:void(0)" class="tnx-action" data-type="deleted" data-id="' + o + '"><em class="fas fa-trash-alt"></em>Delete</a></li>')),
                "deleted" == n && t("#tnx-item-" + o).fadeOut(400, function() {
                    t(this).remove()
                })),
                e.parents("div.modal").modal("toggle"),
                a.remove(),
    ...
    t(document).on("click", ".tnx-transfer-action", function(e) {
        ...
        swal({
            ...
            null == e && "" != e || t.post(transfer_action_url, a.data()).done(function(t) {
                ...
                show_toast(t.msg, t.message, e),
                "success" == t.msg && setTimeout(function() {
                    window.location.reload()
                }, 1200)
            }).fail(function(t, e, a) {
                _log(t, e, a),
    ...
    if (t(document).on("click", "#adjust_token", function(a) {
        ...
        e.html(""),
        t.post(trnx_adjust_url, {
            _token: csrf_token,
            tnx_id: n
        }).done(t=>{
            console.log(t),
            t.status && "die" == t.status ? show_toast(t.msg, t.message, "ti ti-lock") : (e.empty().html(t.modal),
            init_inside_modal(),
            e.children(".modal").modal("show"))
        }
        ).fail(function(t, e, a) {
    ...
    t(".wh-upload-zone").length > 0) {
        ...
        if (t(".whitepaper_upload").length > 0) {
            ...
            E.on("success", function(t, e) {
                cl(e);
                var a = e.message;
                "danger" == e.msg && (alert(a),
                E.removeFile(t)),
                show_toast(e.msg, e.message, "ti ti-filter")
            })
        }
    }
    t(document).on("click", "a.get_page", function(a) {
        ...
        t.post(view_page_url, {
            ...
            cl(a),
            "error" == a.msg && (t(".faq-" + get_id).fadeOut(400, function() {
                t(this).remove()
            }),
    ...
    t(document).on("click", ".wallet-change-action", function(e) {
        ...
        swal({
            ...
            e && t.post(wallet_change_url, {
                ...
                cl(e),
                "success" == e.msg && t(".request-" + a).hide(400),
                show_toast(e.msg, e.message)
            }
            )
        }
        )
    ...
    var V = t(".date-opt")
      , X = t(".date-hide-show");
    V.length > 0 && ("custom" == V.val() && X.show(),
