$(document).ready(function(){
    ...
    function filteringItems(actionDom,item){
        if(actionDom.val() != ''){
            item.find('label:contains("'+ actionDom.val() +'")').closest('.checkbox').show();
            item.find('label:not(:contains("'+ actionDom.val() +'"))').closest('.checkbox').hide();
        }
        else{
	...
	$('#add-route').on('click', function(){
		$('#all-routes').find('.route-check-box:checked').each(function(){
			var $this = $(this);
			var itemName = $(this).siblings('label').text();
    ...
    function formatMenuData(){
        var serializedData= $('#menu-form').serializeArray();
        var jsonData ={};
        var regex = /\[([^\]]+)\]/g;
        serializedData.forEach(function(value, key){
            var match = value.name.match(regex);
            if(match && match.length===2){
                var trim0=trimString(match[0]);
                var trim1= trimString(match[1]);
                if(!jsonData[trim0]){
                    jsonData[trim0] = {};
                }
