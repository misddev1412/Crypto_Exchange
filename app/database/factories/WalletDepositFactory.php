...
$factory->define(WalletDeposit::class, function (Faker $faker) {
    ...
    if (is_array($wallet->coin->api['selected_apis']) && in_array(API_BANK, $wallet->coin->api['selected_apis'])) {
        ...
        $systemBankAccountId = $faker->randomElement($wallet->coin->api['selected_banks']);
        $address = null;
    }
    return [
        'user_id' => $user->id,
        ...
        'status' => $faker->boolean(60) ? STATUS_COMPLETED : ($bankAccountId ? STATUS_REVIEWING : STATUS_COMPLETED),
