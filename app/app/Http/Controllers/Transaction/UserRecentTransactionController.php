...
class UserRecentTransactionController extends Controller
{
    public function index(User $user)
    {
        $exchange = Exchange::where('user_id', $user->id)
            ->select([
                'id',
                'total as amount',
                'coin',
        ...
        $transactions = WalletDeposit::where('user_id', $user->id)
            ->where('status', STATUS_COMPLETED)
            ->select(
                'id',
                'amount',
        ...
        return view('transactions.user_recent_transactions', $data);
    }
}
